        ...
        public void InitLuaConfigs()
        {
            if (m_LauncherComponent.EditorResourceMode)
            {
                ...
                foreach (string path in luaFilePaths)
                {
                    ...
                    {
                        Log.Error(
                            $"[Lua] 发现重名Lua脚本，请检查路径：{m_LuaScriptsFullPathsMapForLoading[luaFileTidyName]}，路径2：{luaFileFullPath}。");
                    }
                }
            }
            else
            {
                ...
                foreach (string key in m_AssetComponent.DependsDataList.Keys)
                {
                    ...
                    {
                        allPathsUnderGame.Add(
                            m_AssetComponent.AssetLoadManager.AssetBundleLoadManager.GetABRestoredPath(key));
                    }
                }
