    ...
    public sealed partial class UIManager
    {
        ...
        private void RefreshTextComponentsAdaptationParams(GameObject go, List<LocalizationFontData> fontDatas)
        {
            ...
            for (int tlcIndex = 0; tlcIndex < textLocalizingComponents.Length; tlcIndex++)
            {
                ...
                //}
                if (aorTextLocalizingComponent.Language != GameDefinitions.Language.Unspecified && aorTextLocalizingComponent.Language != GameMainRoot.Localization.Language)
                {
                    m_LocalizationComponent.GetFontDatas(aorTextLocalizingComponent.Language, out List<LocalizationFontData> lastFontDatas);
                ...
                if (targetFontType == GameDefinitions.AssetType.Font)
                {
                    ...
                    if(textComponent)
                    {
                        ...
                        aorTextLocalizingComponent.Language = GameMainRoot.Localization.Language;
                        // 校正坐标
                        if (aorTextLocalizingComponent.OpenPosOffset)
                        {
                    ...
                    if(tmpComponent)
                    {
                        ...
                        if (tmpComponent.fontSharedMaterial.name.Contains(contactFlag))
                        {
                            ...
                            if (idx > 0)
                            {
                                ...
                                {
                                    targetFontMaterial = (Material)m_AssetComponent.LoadAssetSync("Material", targetFontData.ABPath, $"{targetFont.name}{contactFlag}{targetFontNameSuffix}");
                                }
                            }
                        }
                        ...
                        if (targetFontMaterial)
                        {
                            ...
                            m_AssetComponent.UnloadAsset(targetFontMaterial);
                        }
                        if (aorTextLocalizingComponent.OpenPosOffset)
                        {
