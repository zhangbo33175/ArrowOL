...
namespace Honor.Runtime
{
    ...
    public class AorListView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        List<AorListViewItem> mItemList = new List<AorListViewItem>();
        ...
        // by taoye
        OnListViewGetItemByIndex OnGetItemByIndex;
        ...
        AorItemPosMgr _mAorItemPosMgr = null;
        ...
        AorClickEventListener _mScrollBarAorClickEventListener = null;
        SnapData mCurSnapData = new SnapData();
        Vector3 mLastSnapCheckPos = Vector3.zero;
        bool mListViewInited = false;
        // by taoye
        public bool IsInited { get => mListViewInited; }
        ...
        public int MaxItemNum { get => mMaxItemNum; }

        // by taoye
        [SerializeField]
        private bool mIsPageView;
        public bool IsPageView { get => mIsPageView; }

        // by taoye
        [SerializeField]
        private Transform mDotsRoot;
        ...
        public List<AorListViewItem> ItemList
        {
            get
            {
                return mItemList;
        ...
        /*
        InitListView method is to initiate the ListView component. There are 3 parameters:
        ...
        */
        public void InitListView(int itemTotalCount, OnListViewGetItemByIndex onGetItemByIndex, ListViewInitParam initParam = null)
        {
            if(initParam != null)
            {
            ...
            {
                Debug.LogError("ListView.InitListView method can be called only once.");
            }
        ...
        {
            int curNearestItemIndex = CurSnapNearestItemIndex;
            if (curNearestItemIndex < 0 || curNearestItemIndex >= mMaxItemNum)
            {
                return;
            }
        ...
        void OnPageEndDrag()
        {
            ...
            int curNearestItemIndex = CurSnapNearestItemIndex;
            AorListViewItem item = GetShownItemByItemIndex(curNearestItemIndex);
            if (item == null)
            {
                ClearSnapData();
                return;
            ...
            {
                SetSnapTargetItemIndex(curNearestItemIndex);
                return;
            }
            ...
            if (pos.x > 0)
            {
                if (vec > 0)
                {
                    SetSnapTargetItemIndex(curNearestItemIndex - 1);
                }
                else
                {
                    SetSnapTargetItemIndex(curNearestItemIndex);
        ...
        */
        ...
        /*
         All visible items is stored in a List<ListViewItem> , which is named mItemList;
        ...
        */
        ...
        public AorListViewItem GetShownItemByIndexWithoutCheck(int index)
        {
            return mItemList[index];
        }
        ...
        /*
        For a vertical scrollrect, when a visible item’s height changed at runtime, then this method should be called to let the ListView component reposition all visible items’ position.
        For a horizontal scrollrect, when a visible item’s width changed at runtime, then this method should be called to let the ListView component reposition all visible items’ position.
        */
        ...
        */
        public void RefreshItemByItemIndex(int itemIndex)
        {
            ...
            AorListViewItem curItem = mItemList[i];
        ...
        */
        ...
        void RecycleItemTmp(AorListViewItem item)
        {
            if (item == null)
            {
                return;
        ...
        void RecycleAllItem()
        {
            foreach (AorListViewItem item in mItemList)
            {
                RecycleItemTmp(item);
            }
            mItemList.Clear();
        ...
        void InitItemPool()
        {
            foreach (ItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                AdjustAnchor(rtf);
                AdjustPivot(rtf);
                AorListViewItem tItem = data.mItemPrefab.GetComponent<AorListViewItem>();
                if (tItem == null)
                {
                    data.mItemPrefab.AddComponent<AorListViewItem>();
                }
        ...
        AorListViewItem GetNewItemByIndex(int index)
        {
            ...
            AorListViewItem newItem = OnGetItemByIndex(index);
            if (newItem == null)
            {
                return null;
            }
        ...
        {
            return _mAorItemPosMgr.GetItemIndexAndPosAtGivenPos(pos, ref index, ref itemPos);
        }


        float GetItemPos(int itemIndex)
        {
            return _mAorItemPosMgr.GetItemPos(itemIndex);
        }
        ...
        void AdjustPanelPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                AorListViewItem tViewItem0 = mItemList[0];
                ...
                AorListViewItem tViewItem1 = mItemList[mItemList.Count - 1];
        ...
        void UpdateSnapVertical(bool immediate = false, bool forceSendEvent = false)
        {
            ...
            if (needCheck)
            {
                ...
                if (curIndex >= 0)
                {
                    ...
                    {
                        if (OnListViewSnapNearestChanged != null)
                        {
                            UpdateAllDots();
                            OnListViewSnapNearestChanged(mItemList[curIndex]);
                        }
                    }
                }
                else
            ...
            if (immediate || Mathf.Abs(mCurSnapData.mTargetSnapVal - mCurSnapData.mCurSnapVal) < mSnapFinishThreshold)
            {
                ...
                if (OnListViewSnapItemFinished != null)
                {
                    AorListViewItem targetItem = GetShownItemByItemIndex(mCurSnapNearestItemIndex);
                    if(targetItem != null)
                    {
                        OnListViewSnapItemFinished(targetItem);
                    }
                }
            }
            else
        ...
        void UpdateCurSnapData()
        {
            ...
            if (mCurSnapData.mSnapStatus == SnapStatus.SnapMoving)
            {
                if(mCurSnapData.mIsForceSnapTo)
                {
                    ...
                    {
                        AorListViewItem targetItem = GetShownItemNearestItemIndex(mCurSnapData.mSnapTargetIndex);
                        if (targetItem == null)
                        {
                            mCurSnapData.Clear();
                            return;
            ...
            {
                AorListViewItem nearestItem = GetShownItemByItemIndex(mCurSnapNearestItemIndex);
                if (nearestItem == null)
                {
                    return;
                }
        ...
        //moveMaxAbsVec param is the max abs snap move speed, if the value <= 0 then ListView would use SnapMoveDefaultMaxAbsVec
        ...
        bool CanSnap()
        {
            ...
            if (_mScrollBarAorClickEventListener != null)
            {
                if (_mScrollBarAorClickEventListener.IsPressd)
                {
                    return false;
                }
            }
        ...
        bool UpdateForVertList(float distanceForRecycle0,float distanceForRecycle1,float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                if (itemListCount == 0)
                {
                    ...
                    AorListViewItem newItem = GetNewItemByIndex(index);
                    if (newItem == null)
                    {
                        return false;
                    }
                ...
                if (mViewPortRectLocalCorners[0].y - downPos1.y < distanceForNew1)
                {
                    ...
                    {
                        AorListViewItem newItem = GetNewItemByIndex(nIndex);
                        if (newItem == null)
                        {
                            mCurReadyMaxItemIndex = tViewItem1.ItemIndex;
                            mNeedCheckNextMaxItem = false;
        ...
        float GetContentPanelSize()
        {
            if (mSupportScrollBar)
            {
                float tTotalSize = _mAorItemPosMgr.mTotalSize > 0 ? (_mAorItemPosMgr.mTotalSize - mLastItemPadding) : 0;
                if(tTotalSize < 0)
                {
                    tTotalSize = 0;
                }
        ...
        void CheckIfNeedUpdataItemPos()
        {
            ...
            {
                AorListViewItem firstItem = mItemList[0];
                AorListViewItem lastItem = mItemList[mItemList.Count - 1];
        ...
        void UpdateAllShownItemsPos()
        {
            ...
            float deltaTime = Time.deltaTime == 0f ? GameMainRoot.Launcher.GameSpeedCache / GameMainRoot.Launcher.FrameRate : Time.deltaTime;
