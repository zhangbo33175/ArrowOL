    ...
    public static class GameBinaryExtension
    {
        ...
        public static int Read7BitEncodedInt32(this BinaryReader binaryReader)
        {
            ...
            do
            {
                if (shift >= 35)
                {
                    throw new GameException("7 bit encoded int 无效。");
                }
        ...
        public static string ReadEncryptedString(this BinaryReader binaryReader, byte[] encryptBytes)
        {
            ...
            Encryption.GetSelfXorBytes(s_CachedBytes, encryptBytes, length);
            string value = Converter.GetString(s_CachedBytes, 0, length);
        ...
        public static void WriteEncryptedString(this BinaryWriter binaryWriter, string value, byte[] encryptBytes)
        {
            ...
            byte[] encryptedStringBytes = Converter.GetBytesByString(value);
            if (encryptedStringBytes.Length > byte.MaxValue)
            {
                throw new GameException(AorTxt.Format("String '{0}' is too long.", value));
            }

            Encryption.GetSelfXorBytes(encryptedStringBytes, encryptBytes);
