...
namespace Honor.Editor
{
    ...
    internal sealed class ProcedureComponentInspector : HonorComponentInspector
    {
        ...
        private void OnEnable()
        {
            ...
            m_ProcedureTransitionEnterBlockRaycasts =
                serializedObject.FindProperty("m_ProcedureTransitionEnterBlockRaycasts");
            ...
            m_ProcedureTransitionExitBlockRaycasts =
                serializedObject.FindProperty("m_ProcedureTransitionExitBlockRaycasts");
            m_ProcedureTransitionEnterFlagFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionEnterFlagFromProcedureHotfix");
            m_ProcedureTransitionEnterDurationFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionEnterDurationFromProcedureHotfix");
            m_ProcedureTransitionEnterBlockRaycastFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionEnterBlockRaycastFromProcedureHotfix");
            m_ProcedureTransitionExitFlagFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionExitFlagFromProcedureHotfix");
            m_ProcedureTransitionExitDurationFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionExitDurationFromProcedureHotfix");
            m_ProcedureTransitionExitBlockRaycastFromProcedureHotfix =
                serializedObject.FindProperty("m_ProcedureTransitionExitBlockRaycastFromProcedureHotfix");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlayingOrWillChangePlaymode);
            {
                ...
                if (m_ProcedureTypeNames.arraySize > 0)
                {
                    EditorGUILayout.BeginVertical("box");
                    {
                        for (int i = 0; i < m_ProcedureTypeNames.arraySize; i++)
                        {
                            ...
                            EditorGUILayout.BeginVertical("box");
                            EditorGUILayout.LabelField(AorTxt.Format("[{0}]",
                                m_ProcedureTypeNames.GetArrayElementAtIndex(i).stringValue));
                            ...
                            if (procedureName == "Honor.Runtime.ProcedurePreload")
                            {
                                ...
                                m_ProcedureTransitionEnterFlags.GetArrayElementAtIndex(i).boolValue =
                                    EditorGUILayout.ToggleLeft("进入式-切换过渡（前次流程非热更新时有效）",
                                        m_ProcedureTransitionEnterFlags.GetArrayElementAtIndex(i).boolValue,
                                        new[] { GUILayout.Width(350) });
                                EditorGUI.BeginDisabledGroup(!m_ProcedureTransitionEnterFlags.GetArrayElementAtIndex(i)
                                    .boolValue);
                                {
                                    GUILayout.FlexibleSpace();
                                    EditorGUILayout.LabelField("过渡时间", new[] { GUILayout.Width(60) });
                                    m_ProcedureTransitionEnterDurations.GetArrayElementAtIndex(i).floatValue =
                                        EditorGUILayout.FloatField(
                                            m_ProcedureTransitionEnterDurations.GetArrayElementAtIndex(i).floatValue,
                                            new[] { GUILayout.Width(60) });
                                    GUILayout.FlexibleSpace();
                                    m_ProcedureTransitionEnterBlockRaycasts.GetArrayElementAtIndex(i).boolValue =
                                        EditorGUILayout.ToggleLeft("阻塞射线",
                                            m_ProcedureTransitionEnterBlockRaycasts.GetArrayElementAtIndex(i).boolValue,
                                            new[] { GUILayout.Width(100) });
                                }
                                EditorGUI.EndDisabledGroup();
                                EditorGUILayout.EndHorizontal();

                                EditorGUILayout.BeginHorizontal("box", new[] { GUILayout.Width(500) });
                                m_ProcedureTransitionExitFlags.GetArrayElementAtIndex(i).boolValue =
                                    EditorGUILayout.ToggleLeft("退出式-切换过渡（前次流程非热更新时有效）",
                                        m_ProcedureTransitionExitFlags.GetArrayElementAtIndex(i).boolValue,
                                        new[] { GUILayout.Width(350) });
                                EditorGUI.BeginDisabledGroup(!m_ProcedureTransitionExitFlags.GetArrayElementAtIndex(i)
                                    .boolValue);
                                {
                                    GUILayout.FlexibleSpace();
                                    EditorGUILayout.LabelField("过渡时间", new[] { GUILayout.Width(60) });
                                    m_ProcedureTransitionExitDurations.GetArrayElementAtIndex(i).floatValue =
                                        EditorGUILayout.FloatField(
                                            m_ProcedureTransitionExitDurations.GetArrayElementAtIndex(i).floatValue,
                                            new[] { GUILayout.Width(60) });
                                    GUILayout.FlexibleSpace();
                                    m_ProcedureTransitionExitBlockRaycasts.GetArrayElementAtIndex(i).boolValue =
                                        EditorGUILayout.ToggleLeft("阻塞射线",
                                            m_ProcedureTransitionExitBlockRaycasts.GetArrayElementAtIndex(i).boolValue,
                                            new[] { GUILayout.Width(100) });
                                }
                                ...
                                m_ProcedureTransitionEnterFlagFromProcedureHotfix.boolValue =
                                    EditorGUILayout.ToggleLeft("进入式-切换过渡（前次流程为热更新时有效）",
                                        m_ProcedureTransitionEnterFlagFromProcedureHotfix.boolValue,
                                        new[] { GUILayout.Width(350) });
                                EditorGUI.BeginDisabledGroup(!m_ProcedureTransitionEnterFlagFromProcedureHotfix
                                    .boolValue);
                                {
                                    GUILayout.FlexibleSpace();
                                    EditorGUILayout.LabelField("过渡时间", new[] { GUILayout.Width(60) });
                                ...
                                m_ProcedureTransitionExitFlagFromProcedureHotfix.boolValue =
                                    EditorGUILayout.ToggleLeft("退出式-切换过渡（前次流程为热更新时有效）",
                                        m_ProcedureTransitionExitFlagFromProcedureHotfix.boolValue,
                                        new[] { GUILayout.Width(350) });
                            ...
                            if (ProcedureComponent.LuaScriptWhiteNameList.Contains(luaScriptName))
                            {
                                if (GUILayout.Button("生成/刷新Lua脚本"))
                                {
                                    ...
                                    string[] fileFullPaths = System.IO.Directory.GetFiles(luaRootPath,
                                        AorTxt.Format("{0}.lua.txt", luaScriptName),
                                        System.IO.SearchOption.AllDirectories);
                                    // luaRootPath目录及其子目录中没有该名称的lua文件时需要调起保存窗口
                                    if (fileFullPaths.Length == 0)
                                    {
                                        string exportFileName = EditorUtility.SaveFilePanel(
                                            AorTxt.Format("生成{0}.lua.txt文件", luaScriptName), luaRootPath,
                                            AorTxt.Format("{0}.lua.txt", luaScriptName), string.Empty);
                                        if (!string.IsNullOrEmpty(exportFileName))
                                        {
                                            try
                                            {
                                                StringBuilder stringBuilderCommentLines =
                                                    GenerateCommentLines(luaScriptName);
                                                StringBuilder stringBuilderEmptyCodeLines =
                                                    GenerateEmptyCodeLines(luaScriptName);
                                                string content = AorTxt.Format("{0}{1}",
                                                    stringBuilderCommentLines.ToString(),
                                                    stringBuilderEmptyCodeLines.ToString());
                                                System.IO.File.WriteAllText(exportFileName, content,
                                            ...
                                            {
                                                Log.Error(AorTxt.Format("生成{0}文件失败, 异常信息： '{1}'.", exportFileName,
                                                    exception.ToString()));
                                            }
                                        }
                                    }
                                    ...
                                    else if (fileFullPaths.Length == 1)
                                    {
                                        try
                                        {
                                            StringBuilder stringBuilderCommentLines =
                                                GenerateCommentLines(luaScriptName);
                                            StringBuilder stringBuilderCodeLines =
                                                GenerateCodeLines(luaScriptName, fileFullPaths[0]);
                                            string content = AorTxt.Format("{0}{1}",
                                                stringBuilderCommentLines.ToString(),
                                                stringBuilderCodeLines.ToString());
                                            System.IO.File.WriteAllText(fileFullPaths[0], content,
                                                new System.Text.UTF8Encoding(false));
                                        ...
                                        {
                                            Log.Error(AorTxt.Format("刷新{0}文件失败, 异常信息： '{1}'.", fileFullPaths[0],
                                                exception.ToString()));
                                        }
                                    }
                                    else
                                    {
                                        Log.Error(AorTxt.Format("目录 {0} 及其子目录中遍历到多个文件 {1}.txt ！", luaRootPath,
                                            luaScriptName));
                                    }
                ...
                if (m_ProcedureTypeNames.arraySize > 0)
                {
                    ...
                    int selectedIndex =
                        EditorGUILayout.Popup("入口流程", m_EntryProcedureIndex, procedureTypeNames.ToArray());
            ...
            m_UITransitionAssetName.stringValue =
                EditorGUILayout.TextField("切换过渡界面资源名称", m_UITransitionAssetName.stringValue);
            ...
            EditorGUI.BeginDisabledGroup(!m_UseUIPreload.boolValue);
            {
                ...
                m_UIPreloadAssetName.stringValue =
                    EditorGUILayout.TextField("预加载界面资源名称", m_UIPreloadAssetName.stringValue);
            ...
            if (Application.isPlaying)
            {
                ...
                for (int index = 0; index < t.RuntimeProcedureRecordInfos.Count; index++)
                {
                    ...
                    {
                        EditorGUILayout.LabelField(AorTxt.Format("{0}\t（{1:N2}秒）",
                            t.CurrentProcedure.GetType().ToString(), t.CurrentProcedureTime));
                    }
                }

                if (GUILayout.Button("导出 CSV 数据"))
                {
                    string exportFileName = EditorUtility.SaveFilePanel("导出 CSV 数据", string.Empty,
                        AorTxt.Format("流程跳转信息 {0}.csv", DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss")), string.Empty);
                    if (!string.IsNullOrEmpty(exportFileName))
                    {
                        try
                        {
                            ...
                            data[index] = AorTxt.Format("{0}\t（{1:N2}秒）", t.CurrentProcedure.GetType().ToString(),
                                t.CurrentProcedureTime);
                        ...
                        {
                            Log.Error(AorTxt.Format("导出 CSV 数据到 '{0}' 失败, 异常信息： '{1}'.", exportFileName,
                                exception.ToString()));
                        }
                    }
        ...
        private StringBuilder GenerateCodeLines(string luaScriptName, string fullPath)
        {
            ...
            commentDefs.Add(new List<string>(new string[]
            {
                AorTxt.Format("---构造函数"), AorTxt.Format("---@type fun(args:table):void"),
                AorTxt.Format("---@param args table @自定义参数")
            }));
            commentDefs.Add(new List<string>(new string[]
            {
                AorTxt.Format("---创建函数"), AorTxt.Format("---@type fun(args:table):{0}", luaName),
                AorTxt.Format("---@param args table @自定义参数"), AorTxt.Format("---@return {0} @流程实例", luaName)
            }));
            commentDefs.Add(new List<string>(new string[]
            {
                AorTxt.Format("---进入"), AorTxt.Format("---@type fun(ownerMachine: userdata):void"),
                AorTxt.Format("---@param ownerMachine userdata @流程状态")
            }));
            commentDefs.Add(new List<string>(new string[]
            {
                AorTxt.Format("---心跳"), AorTxt.Format("---@type fun(ownerMachine: userdata):void"),
                AorTxt.Format("---@param ownerMachine userdata @流程状态")
            }));
            commentDefs.Add(new List<string>(new string[]
            {
                AorTxt.Format("---离开"), AorTxt.Format("---@type fun(ownerMachine: userdata):void"),
                AorTxt.Format("---@param ownerMachine userdata @流程状态")
            }));
