...
namespace Honor.Editor
{
    ...
    public class LauncherComponentInspector : HonorComponentInspector
    {
        ...
        // ======================= 序列化字段 =======================
        private SerializedProperty m_EditorResourceMode; // 编辑器资源模式
        private SerializedProperty m_EditorLanguage; // 编辑器语言
        private SerializedProperty m_DevelopMode; // 开发模式
        private SerializedProperty m_IsLocalServer; // 本地服务器
        private SerializedProperty m_IsRealTimeDebuggerHotfixForEditor; // 实时调试热更新
        private SerializedProperty m_LuacMode; // Lua字节码模式
        private SerializedProperty m_LuaHotReloadMode; // Lua热重载
        private SerializedProperty m_DebuggerActiveWindow; // 调试窗口激活
        private SerializedProperty m_FrameRate; // 帧率
        private SerializedProperty m_GameSpeed; // 游戏倍速
        private SerializedProperty m_RunInBackground; // 后台运行
        private SerializedProperty m_NeverSleep; // 屏幕常亮
        private GameDefinitions.DebugMode m_LuaDebugMode; // Lua调试模式
        private SerializedProperty m_CustomDevicePerformance; // 自定义设备性能
        private SerializedProperty m_UseDevicePerformance; // 使用设备性能分级
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!m_EditorResourceMode.boolValue) EditorGUI.BeginDisabledGroup(true);
            {
                ...
                if (Application.isPlaying)
                {
                    ...
                    EditorGUILayout.LabelField("编辑器启动语言",
                        $"{langName}({GameDefinitions.LanguageDesc[m_EditorLanguage.enumValueIndex]})");
                }
                else
                {
                    m_EditorLanguage.enumValueIndex =
                        EditorGUILayout.Popup("编辑器启动语言", m_EditorLanguage.enumValueIndex, langList);
                }
            }
            if (!m_EditorResourceMode.boolValue) EditorGUI.EndDisabledGroup();
            ...
            if (Application.isPlaying) EditorGUI.BeginDisabledGroup(true);
            {
                ...
                {
                    GameDefinitions.DebugMode newDebugMode =
                        (GameDefinitions.DebugMode)EditorGUILayout.EnumPopup("Lua调试", m_LuaDebugMode);
            ...
            {
                if (EditorApplication.isPlaying) t.FrameRate = frameRate;
                else m_FrameRate.intValue = frameRate;
            }
            ...
            EditorGUILayout.BeginVertical("box");
            {
                ...
                if (speedIndex >= 0) gameSpeed = GetGameSpeed(speedIndex);

                if (gameSpeed != m_GameSpeed.floatValue)
                {
                    if (EditorApplication.isPlaying) t.GameSpeed = gameSpeed;
                    else m_GameSpeed.floatValue = gameSpeed;
                }
            }
            ...
            {
                if (EditorApplication.isPlaying) t.RunInBackground = runInBackground;
                else m_RunInBackground.boolValue = runInBackground;
            }
            ...
            {
                if (EditorApplication.isPlaying) t.NeverSleep = neverSleep;
                else m_NeverSleep.boolValue = neverSleep;
            }

            // ====================== 设备性能分级 ======================
            m_UseDevicePerformance.boolValue = EditorGUILayout.Toggle("开启硬件性能分级/评级", m_UseDevicePerformance.boolValue);
            m_CustomDevicePerformance.boolValue =
                EditorGUILayout.Toggle("自定义硬件性能分级/评级", m_CustomDevicePerformance.boolValue);
            ...
            if (m_CustomDevicePerformance.boolValue)
            {
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    {
                        string input = EditorGUILayout.TextField($"{PlatformStrings[i]}-CPU核心数：",
                            m_ProcessorCounts[i].intValue.ToString());
                    ...
                    // 高端机
                    EditorGUILayout.Separator();
                    EditorGUILayout.LabelField("高端机",
                        new GUIStyle
                        {
                            normal =
                            {
                                textColor =
                                    DevicePerformance.GetDevicePerformanceLevelColor(DevicePerformanceLevel.High)
                            }
                        });
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        string gfxHigh = EditorGUILayout.TextField("",
                            m_GraphicsMemorySizeHighBases[i].intValue.ToString(), GUILayout.Width(100));
                        if (int.TryParse(gfxHigh, out newData)) m_GraphicsMemorySizeHighBases[i].intValue = newData;

                        EditorGUILayout.LabelField("内存>=", GUILayout.Width(60));
                        string memHigh = EditorGUILayout.TextField("",
                            m_SystemMemorySizeHighBases[i].intValue.ToString(), GUILayout.Width(100));
                        if (int.TryParse(memHigh, out newData)) m_SystemMemorySizeHighBases[i].intValue = newData;
                        EditorGUILayout.EndHorizontal();
                    }

                    // 中端机
                    EditorGUILayout.Separator();
                    EditorGUILayout.LabelField("中端机",
                        new GUIStyle
                        {
                            normal =
                            {
                                textColor = DevicePerformance.GetDevicePerformanceLevelColor(DevicePerformanceLevel.Mid)
                            }
                        });
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        EditorGUILayout.LabelField(PlatformStrings[i], GUILayout.Width(60));
                        EditorGUILayout.LabelField("显存>=", GUILayout.Width(60));
                        string gfxMid = EditorGUILayout.TextField("",
                            m_GraphicsMemorySizeMidBases[i].intValue.ToString(), GUILayout.Width(100));
                        if (int.TryParse(gfxMid, out newData)) m_GraphicsMemorySizeMidBases[i].intValue = newData;

                        EditorGUILayout.LabelField("内存>=", GUILayout.Width(60));
                        string memMid = EditorGUILayout.TextField("", m_SystemMemorySizeMidBases[i].intValue.ToString(),
                            GUILayout.Width(100));
                        if (int.TryParse(memMid, out newData)) m_SystemMemorySizeMidBases[i].intValue = newData;
                        EditorGUILayout.EndHorizontal();
                    }

                    // 低端机自动判定
                    EditorGUILayout.Separator();
                    EditorGUILayout.LabelField("低端机",
                        new GUIStyle
                        {
                            normal =
                            {
                                textColor = DevicePerformance.GetDevicePerformanceLevelColor(DevicePerformanceLevel.Low)
                            }
                        });
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        EditorGUILayout.LabelField(
                            $"显存 < {m_GraphicsMemorySizeMidBases[i].intValue} 或 内存 < {m_SystemMemorySizeMidBases[i].intValue}");
                        EditorGUILayout.EndHorizontal();
                    }
                }
                EditorGUILayout.EndVertical();
        ...
        private void ReadLuaDebugModeFromLibraryConfigFile()
        {
            ...
            else WriteDebugModeToLibraryConfigFile();
        }
