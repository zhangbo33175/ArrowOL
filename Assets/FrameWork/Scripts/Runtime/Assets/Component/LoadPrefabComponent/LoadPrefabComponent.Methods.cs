...
namespace Honor.Runtime
{
    public  partial class LoadPrefabComponent
    {
        ...
        /// <returns>实例</returns>
        private  GameObject Instance_Prefab_GO(LoadPrefabData prefabObj, Transform parent, LuaTable luaParams)
        {
            ...
            {
                tempParent = GameMain.Instance().GetLayerWithLauncher();
            }

            //RGame.RProfiler.BeginSample("PrefabLoadManager.LoadSync.InstanceGO " + prefabObj.AssetName);
            ...
            */
            ...
            // RGame.RProfiler.EndSample("PrefabLoadManager.LoadSync.InstanceGO " + prefabObj.AssetName);
        ...
        /// 实例化对象并回调
        /// </summary>
        private  void Instance_Prefab_GO_With_Callback(LoadPrefabData prefabObj)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    GameObject go = Instance_Prefab_GO(prefabObj, callParentList[i], luaParamList[i]);
        ...
        private  void UpdateLoadedAsync()
        {
            ...
            if (AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList.Count == 0) return;

            int count = AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList.Count;
            for (int i = 0; i < count; i++)
            {
                // 异步回调前需要最大程度的收集接下来需要进行回调的数量
                AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList[i].LockCallbackCount =
                    AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList[i].PrefabLoadOverCallbackList.Count;
            }

            for (int i = 0; i < count; i++)
            {
                Instance_Prefab_GO_With_Callback(AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList[i]);
            }

            AssetManager.Instance.m_LoadedPrefabAsyncTmpAgentList.RemoveRange(0, count);
        }
