...
namespace Honor.Runtime
{
    public class AorTextLocalizing : MonoBehaviour
    {
        ...
        void Start()
        {
            EventManager.Instance.AddEvent(EventCmd.TextLocalizingRefresh, this, RefreshTextValue);
        ...
        void OnDestroy()
        {
            EventManager.Instance.RemoveEvent(EventCmd.TextLocalizingRefresh, this, RefreshTextValue);
        }

        /// <summary>
        /// 刷新文本内容
        /// </summary>
        private void RefreshTextValue(object sender = null, object userData = null, EventManager.EventParams e = null)
        {
            if (sender != null && sender != (object)LocalizationManager.Instance()) return;
            
            if (!string.IsNullOrEmpty(m_LocalizingKeyName))
            {
                m_LocalizingValue = LuaManager.Instance.LuaLocalizingCSEventDelegate(m_LocalizingKeyName);
