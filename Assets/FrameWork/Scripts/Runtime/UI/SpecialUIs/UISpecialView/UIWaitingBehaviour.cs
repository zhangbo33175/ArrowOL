...
{
    public class UIWaitingBehaviour: MonoBehaviour
    {
         /// <summary>
        /// 菊花图片
        /// </summary>
        ...
        private void Awake()
        {
            ...
            if (m_DescText != null)
            {
                m_DescText.text = LocalizationManager.Instance().GetDefaultData("Waiting_Desc_Text");
            }

            if (m_DescTextTMP != null)
            {
                m_DescTextTMP.text = LocalizationManager.Instance().GetDefaultData("Waiting_Desc_Text");
            }
