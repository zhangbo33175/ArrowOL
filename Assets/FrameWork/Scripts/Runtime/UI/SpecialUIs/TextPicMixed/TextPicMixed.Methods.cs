...
namespace Honor.Runtime
{
    public partial class TextPicMixed : TextPic
    {
        ...
        private bool CollectIcon(List<IconName> iconList, string name, string abPath, string assetName, float scaleXY, float offsetX, float offsetY)
        {
            ...
            {
#if UNITY_EDITOR
                Sprite sprite = Application.isPlaying ? Root.Asset.LoadAssetSync("Sprite", abPath, assetName) as Sprite : (Sprite)UnityEditor.AssetDatabase.LoadAssetAtPath(GetSpriteRelativeFullPathInEditor(abPath, assetName), typeof(Sprite));
#else
        ...
        private string GetSpriteRelativeFullPathInEditor(string abPath, string assetName)
        {
            ...
            string abFullPath = Txt.Format("{0}{1}", Application.dataPath.Substring(0, Application.dataPath.Length - "Assets".Length), abPath);
            // 如果不是文件，则开始在该目录下进行搜索
            string fullFilePath = Txt.Format("{0}.png", abFullPath);
            if (!File.Exists(fullFilePath))
            {
                if(Directory.Exists(abFullPath))
                {
                    string[] fileFullPaths = Directory.GetFiles(abFullPath, Txt.Format("{0}.png", assetName), SearchOption.AllDirectories);
