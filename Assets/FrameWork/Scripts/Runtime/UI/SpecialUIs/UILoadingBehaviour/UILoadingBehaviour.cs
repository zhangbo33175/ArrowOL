...
{
    public sealed class UILoadingBehaviour : MonoBehaviour
    {
        /// <summary>
        /// 加载模式
        /// </summary>
        ...
        /// 热更新错误UI界面组件
        /// </summary>
       // private UIHotfixErrorBehaviour m_UIHotfixErrorBehaviour;
        ...
        {
            // 注册事件监听-加载进度
            EventManager.Instance.AddEvent(EventCmd.LoadProgress, this, OnLoadProgressEventCallback);

            // 注册事件监听-热更进度
            EventManager.Instance.AddEvent(EventCmd.HotfixProgress, this, OnHotfixProgressEventCallback);

            // 注册事件监听-热更完毕
            EventManager.Instance.AddEvent(EventCmd.HotfixAllOver, this, OnHotfixAllOverEventCallback);

            // 注册事件监听-热更异常
            EventManager.Instance.AddEvent(EventCmd.HotfixError, this, OnHotfixErrorEventCallback);
        }

        private void OnDisable()
        {
            // 注销事件监听-加载进度
            EventManager.Instance.RemoveEvent(EventCmd.LoadProgress, this, OnLoadProgressEventCallback);

            // 注销事件监听-热更进度
            EventManager.Instance.RemoveEvent(EventCmd.HotfixProgress, this, OnHotfixProgressEventCallback);

            // 注销事件监听-热更完毕
            EventManager.Instance.RemoveEvent(EventCmd.HotfixAllOver, this, OnHotfixAllOverEventCallback);

            // 注销事件监听-热更异常
            EventManager.Instance.RemoveEvent(EventCmd.HotfixError, this, OnHotfixErrorEventCallback);

            // 销毁热更错误显示对象
            /*if (m_UIHotfixErrorBehaviour != null)
            {
                Root.UI.HideHotfixError(m_UIHotfixErrorBehaviour);
                m_UIHotfixErrorBehaviour = null;
            }*/
        }

        private void Start()
        {
            if (m_StartButtonText != null) m_StartButtonText.text = IsWebGL() ? "Start" : LocalizationManager.Instance().GetDefaultData("Hotfix_StartButton_Text");
            if (m_StartButtonTextTMP != null) m_StartButtonTextTMP.text = IsWebGL() ? "Start" : LocalizationManager.Instance().GetDefaultData("Hotfix_StartButton_Text");
            if (m_RetryButtonText != null) m_RetryButtonText.text = IsWebGL() ? "Retry" : LocalizationManager.Instance().GetDefaultData("Hotfix_RetryButton_Text");
            if (m_RetryButtonTextTMP != null) m_RetryButtonTextTMP.text = IsWebGL() ? "Retry" : LocalizationManager.Instance().GetDefaultData("Hotfix_RetryButton_Text");
            if (m_CloseButtonText != null) m_CloseButtonText.text = IsWebGL() ? "Close" : LocalizationManager.Instance().GetDefaultData("Hotfix_CloseButton_Text");
            if (m_CloseButtonTextTMP != null) m_CloseButtonTextTMP.text = IsWebGL() ? "Close" : LocalizationManager.Instance().GetDefaultData("Hotfix_CloseButton_Text");
        }
        ...
        public void SetLoadingMode(LoadingMode loadingMode)
        {
            ...
            if (m_LoadingMode == LoadingMode.Preload)
            {
                ...
                RefreshViews(string.Empty, 0f);
                     ...
                     m_LoadingMode == LoadingMode.WebGLLauncher ||
                     m_LoadingMode == LoadingMode.WebGLIncreaser)
            {
                ...
                //RefreshViews(string.Empty, 0f, GameDefinitions.DownloadStep.Idle, Root.Hotfix.AlreadyDownloadedFileNum, Root.Hotfix.NeedDownloadFileTotalNum, Root.Hotfix.AlreadyDownloadedBytes, Root.Hotfix.NeedDownloadTotalBytes);
            }
        }
        ...
        public void RefreshViews(string descContent, float progress, GameDefinitions.DownloadStep downloadStep = GameDefinitions.DownloadStep.None, int curFileNum = 0, int totalFileNum = 0, float curBytes = 0f, float totalBytes = 0f)
        {
            ...
            if (m_LoadingMode == LoadingMode.HotfixLauncher || 
                ...
                m_LoadingMode == LoadingMode.WebGLIncreaser)
            {
                string textReadyDownload = IsWebGL() ? "Ready to download." : LocalizationManager.Instance().GetDefaultData("Hotfix_Ready_Download_Text");
                string textDownloading = IsWebGL() ? "Downloading: " : LocalizationManager.Instance().GetDefaultData("Hotfix_Downloading_Text");
                string textDownloadAllOver = IsWebGL() ? "It's downloaded." : LocalizationManager.Instance().GetDefaultData("Hotfix_Download_AllOver_Text");
                string textDownloadFailedToSkip = IsWebGL() ? "Download failed, ready to enter the game." :LocalizationManager.Instance().GetDefaultData("Hotfix_Download_Failed_To_Skip_Text");
                string textDownloadError = IsWebGL() ? "Download abnormal!Please try again." : LocalizationManager.Instance().GetDefaultData("Hotfix_Download_Error_Text");
        ...
        /// 事件监听回调-加载进度
        /// </summary>
        private void OnLoadProgressEventCallback(object sender, object userData, EventManager.EventParams e)
        {
        ...
        /// 事件监听回调-热更进度
        /// </summary>
        private void OnHotfixProgressEventCallback(object sender, object userData, EventManager.EventParams e)
        {
            if (userData != this) return;
        ...
        /// 事件监听回调-热更完毕
        /// </summary>
        private void OnHotfixAllOverEventCallback(object sender, object userData, EventManager.EventParams e)
        {
            if (userData != this) return;
        ...
        /// 事件监听回调-热更异常
        /// </summary>
        private void OnHotfixErrorEventCallback(object sender, object userData, EventManager.EventParams e)
        {
            if (userData != this) return;

            string fileName = e.GetString("fileName");
            
                Log.Warning("下载异常！请重试。");
                RefreshViews(fileName, m_ProgressValue, GameDefinitions.DownloadStep.Error, m_CurFileNum, m_TotalFileNum, m_CurBytes, m_TotalBytes);
        ...
        {
            if (m_LoadingMode == LoadingMode.WebGLLauncher || m_LoadingMode == LoadingMode.WebGLIncreaser)
            {
                return true;
            }
            return false;
        ...
        public void ClickAIHelp()
        {
            /*string deviceID = Root.SDK.TGAHelper.GetDeviceID();
            string dataStr = $"{{\"DeviceID\"={deviceID ?? "nil"}}}";
            AIHelp.AIHelpSupport.UpdateUserInfo(
                AIHelp.AIHelpSupport.CreateUserConfig(null, null, null, dataStr));

            var language = Root.Localization.LanguageName;
            var languageCode = aiHelpLanguageCodeDic[language];
            if (languageCode == null)
            {
                languageCode = "en";
            }
            AIHelp.AIHelpSupport.UpdateSDKLanguage(languageCode);
            
            //显示AI帮助中心
            var eid = Root.Config.GetString("AIHelpEntranceId", false);
            if (string.IsNullOrEmpty(eid))
            {
                eid = "E001";
            }
            AIHelp.AIHelpSupport.Show(eid);*/
        }
    }
}