...
namespace Honor.Runtime
{
    public partial class LocalizationComponent
    {
        ...
        {
            TextAsset languagesJsonAsset = (TextAsset)AssetManager.Instance.LoadAssetSync("JsonAsset",
                GamePathUtils.Json.GetRootDirectoryRelativePath(), "LocalizationDefaultLanguages");
            JArray jArray = JArray.Parse(languagesJsonAsset.text);
            foreach (var name in jArray)
            {
                m_DefaultLanguages.Add(
                    (GameDefinitions.Language)System.Enum.Parse(typeof(GameDefinitions.Language), name.ToString()));
            }

            AssetManager.Instance.UnloadAsset(languagesJsonAsset, null, true);
        }
        ...
        {
            TextAsset configJsonAsset = (TextAsset)AssetManager.Instance.LoadAssetSync("JsonAsset", abPath, assetName);
            ...
            AssetManager.Instance.UnloadAsset(configJsonAsset, null, true);
        }
        ...
        public void LoadFontDatas(string abPath, string assetName)
        {
            ...
            foreach (var item in jObject)
            {
                GameDefinitions.Language language =
                    (GameDefinitions.Language)Enum.Parse(typeof(GameDefinitions.Language), item.Key);
                string fontType = item.Value["FontType"].ToString();
                int index = 0;
                while (item.Value[$"Mark{index}"] != null &&
                       !string.IsNullOrEmpty(item.Value[$"Mark{index}"].ToString()))
                {
                    ...
                    LocalizationFontData fontData = new LocalizationFontData(fontType, itemMark, itemABPath,
                        itemAssetName, itemCustomMaterialName, itemFontSizeScaleRatio);
