...
{
    public abstract class ProcedureState : State<ProcedureManager>
    {
        /// <summary>
        /// 切换至下一个流程时需要等待最大帧数
        /// </summary>
        ...
        /// 流程状态进入lua绑定回调
        /// </summary>
        protected Action<StateMachine<ProcedureManager>> m_LuaOnEnter;

        /// <summary>
        /// 流程状态心跳更新lua绑定回调
        /// </summary>
        protected Action<StateMachine<ProcedureManager>> m_LuaOnUpdate;

        /// <summary>
        /// 流程状态离开lua绑定回调
        /// </summary>
        protected Action<StateMachine<ProcedureManager>> m_LuaOnLeave;
        ...
        {
            get { return m_PrepareArgsFromChanging; }
        ...
        public bool RemoveAllContentsOnProcedureTransition
        {
            ...
            get { return m_RemoveAllContentsOnProcedureTransition; }
        }
        ...
        public void InitLuaBindings(string luaScriptName)
        {
            ...
            // 获取全局Lua环境
            LuaEnv luaEnv = LuaManager.Instance.Env;
            ...
            LuaManager.Instance.LuaCreateProcedureLuaClassFromCSEventDelegate(m_OwnEnv, m_LuaScriptName);
        ...
        public override void OnInit(StateMachine<ProcedureManager> ownerMachine)
        {
        ...
        /// </summary>
        /// <param name="ownerMachine">流程持有者。</param>
        public override void OnDestroy(StateMachine<ProcedureManager> ownerMachine)
        {
        ...
        /// </summary>
        /// <param name="ownerMachine">流程持有者。</param>
        public override void OnEnter(StateMachine<ProcedureManager> ownerMachine)
        {
            ...
            // 注册事件监听-切换过渡进入结束
            EventManager.Instance.AddEvent(EventCmd.ProcedureTransitionEnterOver, this,
                OnProcedureTransitionEnterOverEventCallback);

            // 注册事件监听-切换过渡退出结束
            EventManager.Instance.AddEvent(EventCmd.ProcedureTransitionExitOver, this,
                OnProcedureTransitionExitOverEventCallback);

            // 显示流程切换过渡进入效果
            ShowProcedureTransitionEnter(!ProcedureManager.Instance.CurrentProcedureTransitionEnterFlag,
                ProcedureManager.Instance.CurrentProcedureTransitionEnterDuration,
                ProcedureManager.Instance.CurrentProcedureTransitionEnterBlockRaycast);
        }
        ...
        /// </summary>
        /// <param name="ownerMachine">流程持有者。</param>
        public override void OnUpdate(StateMachine<ProcedureManager> ownerMachine)
        {
        ...
        public override void OnLeave(StateMachine<ProcedureManager> ownerMachine, bool isShutdown)
        {
            ...
            // 注销事件监听-切换过渡进入结束
            EventManager.Instance.RemoveEvent(EventCmd.ProcedureTransitionEnterOver, this,
                OnProcedureTransitionEnterOverEventCallback);

            // 注销事件监听-切换过渡退出结束
            EventManager.Instance.RemoveEvent(EventCmd.ProcedureTransitionExitOver, this,
                OnProcedureTransitionExitOverEventCallback);
        }
        ...
        public override void ChangeState(StateMachine<ProcedureManager> ownerMachine, Type stateType)
        {
            ownerMachine.Owner.RecordRuntimeProcedureInfos(ownerMachine.CurrentStateName,
                ownerMachine.CurrentStateTime);
        ...
        public void ChangeState(StateMachine<ProcedureManager> ownerMachine, string stateName)
        {
            ChangeState(ownerMachine, Assembly.GetType(AorTxt.Format("Solar.Runtime.{0}", stateName)));
        }
        ...
        /// <param name="e"></param>
        public void OnProcedureTransitionEnterOverEventCallback(object sender, object userData,
            EventManager.EventParams e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void OnProcedureTransitionExitOverEventCallback(object sender, object userData,
            EventManager.EventParams e)
        {
            if (userData != this) return;
            // 移除游戏中所有内容
        ...
        public void PrepareToNextProcedure(Type stateType, LuaTable argsFromChanging = null)
        {
            ...
            ShowProcedureTransitionExit(!ProcedureManager.Instance.CurrentProcedureTransitionExitFlag,
                ProcedureManager.Instance.CurrentProcedureTransitionExitDuration,
                ProcedureManager.Instance.CurrentProcedureTransitionExitBlockRaycast);
        }
        ...
        {
            PrepareToNextProcedure(Assembly.GetType(AorTxt.Format("Solar.Runtime.{0}", stateName)), argsFromChanging);
        }
        ...
        {
            UIManager.Instance.ShowProcedureTransitionEnter(forceOver, duration, blockRaycast);
        }
        ...
        {
            UIManager.Instance.ShowProcedureTransitionExit(forceOver, duration, blockRaycast);
        }
        ...
        private void RemoveAllContents()
        {
            ...
            {
                // 关闭所有UI
                UIManager.Instance.CloseAllUIs(UIType.Screen, true);
                UIManager.Instance.CloseAllUIs(UIType.Scene, true);
                // 销毁所有场景中对象
                //Root.Scene.DestroyAllSceneGOs();
                // 卸载字体（马上，不等待Asset的过期帧数）
                UIManager.Instance.UnloadFonts(true);
            }
        }
    }
}