...
namespace Honor.Runtime
{
    ...
    public abstract class GameSerializer<T>
    {
        ...
        public void RegisterSerializeCallback(byte version, SerializeCallback callback)
        {
            if (callback == null)
            {
                throw new GameException("序列化回调函数无效。");
            }
        ...
        public void RegisterTryGetValueCallback(byte version, TryGetValueCallback callback)
        {
            if (callback == null)
            {
                throw new GameException("尝试从指定流获取指定键的值回调函数无效。");
            }
        ...
        public bool Serialize(Stream stream, T data, string flag = default(string))
        {
            ...
            {
                throw new GameException("无注册过的序列化回调函数。");
            }

            return Serialize(stream, data, m_LatestSerializeCallbackVersion, flag);
        }
        ...
        public T Deserialize(Stream stream, string flag = default(string))
        {
            ...
            {
                byte[] header = GetHeader();
                if (binaryReader.ReadByte() != header[0] || binaryReader.ReadByte() != header[1] ||
