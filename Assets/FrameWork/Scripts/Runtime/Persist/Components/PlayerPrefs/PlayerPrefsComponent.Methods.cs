...
{
    public sealed partial class PlayerPrefsComponent
    {
      /// <summary>
        /// 从存档中读取所有条目
        /// </summary>
        private void LoadItemNameGroups()
        {
            ...
            if (!string.IsNullOrEmpty(classifyNameListText))
            {
                ...
                for (int classifyNameIndex = 0; classifyNameIndex < classifyNameList.Length; classifyNameIndex++)
                {
                    ...
                    string classifyXXXXXItemNameListText = AESEncrypt.DecodeFromBase64(PlayerPrefs.GetString($"Classify_{classifyName}_ItemNameList", AESEncrypt.EncodeToBase64(string.Empty)));
