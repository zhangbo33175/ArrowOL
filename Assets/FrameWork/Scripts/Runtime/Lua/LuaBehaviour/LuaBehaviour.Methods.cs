...
namespace Honor.Runtime
{
    public partial class LuaBehaviour : MonoBehaviour
    {
        ...
        {
            // 实例化Lua脚本的独立环境
            OwnLuaEnvs[index] = LuaManager.Instance.Env.NewTable();
            LuaTable meta = LuaManager.Instance.Env.NewTable();
            ...
            for (int injectionIndex = 0; injectionIndex < m_Injections.Count; injectionIndex++)
            {
                ...
                LuaTable luaEnv = isArray ? LuaManager.Instance.Env.NewTable() : OwnLuaEnvs[index];
                    ...
                    for (int idx = 0; idx < objs.Count; idx++)
                    {
                        if (objs[idx])
                        {
                            if (!string.IsNullOrEmpty(infoExs[idx]))
                            {
                                ...
                                luaEnv.Set(keys[idx],
                                    ((GameObject)objs[idx]).GetComponent(tidyName[tidyName.Length - 1]));
                            }
                            else
                            {
                    ...
                    for (int idx = 0; idx < objs.Count; idx++)
                    {
                        if (objs[idx])
                        {
                            ...
                            if (luaScriptIndex >= 0)
                            {
                                ...
                                {
                                   // luaBehaviour.transform.SetParent(Root.Asset.transform, false);
                                    useTmpParent = true;
                                }
                                ...
                                else
                                {
                                    if (isArray)
                                    {
                                        Log.Error("注入对象 {0}[{1}] 还未初始化luaClass脚本 {2} ！", injection.Name, (int)keys[idx],
                                            infoExs[idx]);
                                    }
                                    else
                                    {
                    ...
                    for (int idx = 0; idx < objs.Count; idx++)
                    {
                        switch (injection.InjectionTypeName)
                        {
                            ...
                            case LuaInjection.InjectionType.UI_TextPicMixed:
                                if (objs[idx] != null) luaEnv.Set(keys[idx], (TextPicMixed)objs[idx]);
                                break;
                            ...
                            case LuaInjection.InjectionType.UI_Tree:
                                if (objs[idx] != null) luaEnv.Set(keys[idx], (Tree)objs[idx]);
                                break;
                            ...
                            case LuaInjection.InjectionType.Float:
                                luaEnv.Set(keys[idx],
                                    System.Convert.ChangeType(variants[idx], System.Type.GetType("System.Single")));
                                break;
                            default:
                                luaEnv.Set(keys[idx],
                                    System.Convert.ChangeType(variants[idx],
                                        System.Type.GetType(AorTxt.Format("System.{0}",
                                            injection.InjectionTypeName.ToString()))));
                                break;
                        }
                    }
                }
            ...
            if (m_ShowRaycastTargetsGizmos)
            {
                ...
                foreach (MaskableGraphic graphic in graphics)
                {
                    if (graphic.raycastTarget)
                    {
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            Gizmos.DrawLine(m_RaycastTargetWorldCornersOnDrawGizmos[i],
                                m_RaycastTargetWorldCornersOnDrawGizmos[(i + 1) % 4]);
                        }
                    }
                }
            }
